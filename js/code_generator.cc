...
CodeGenerator::CodeGenerator(const std::string &name) : name_(name) {
}

CodeGenerator::~CodeGenerator() {
}

bool CodeGenerator::Generate(const google::protobuf::FileDescriptor *file, const std::string&, google::protobuf::compiler::OutputDirectory *output_directory, std::string *error) const {
    ...
    google::protobuf::internal::scoped_ptr< google::protobuf::io::ZeroCopyOutputStream> output(output_directory->Open(output_file_name));
    ...
    for (int i = 0; i < file->message_type_count(); ++i) {
        CodeGenerator::GenDescriptorGoogProvides(file->message_type(i), &printer);
    }
    for (int i = 0; i < file->enum_type_count(); ++i) {
        CodeGenerator::GenEnumDescriptorGoogProvides(file->enum_type(i), &printer);
    }
    ...
    for (int i = 0; i < file->dependency_count(); ++i) {
        for (int j = 0; j < file->dependency(i)->message_type_count(); j++) {
            printer.Print("goog.require('$file$');\n",
                          "file", JsFullName(file->dependency(i)->message_type(j)->file(), file->dependency(i)->message_type(j)->full_name()));
        }

        for (int j = 0; j < file->dependency(i)->enum_type_count(); j++) {
            const std::string enum_name = JsFullName(file->dependency(i)->enum_type(j)->file(), file->dependency(i)->enum_type(j)->full_name());
    ...
    for (int i = 0; i < file->message_type_count(); ++i) {
        CodeGenerator::GenDescriptor(file->message_type(i), &printer);
    }
    for (int i = 0; i < file->enum_type_count(); ++i) {
        CodeGenerator::GenEnumDescriptor(file->enum_type(i), &printer);
    }
    ...
    // generate metadata
    for (int i = 0; i < file->message_type_count(); ++i) {
        CodeGenerator::GenDescriptorMetadata(file->message_type(i), &printer);
    }
...
std::string CodeGenerator::JsFullName(const google::protobuf::FileDescriptor *file, const std::string &full_name) {
...
void CodeGenerator::GenDescriptorGoogProvides(const google::protobuf::Descriptor *message, google::protobuf::io::Printer *printer) {
    printer->Print("goog.provide('$name$');\n",
                   "name", JsFullName(message->file(), message->full_name()));

    // enums
    for (int i = 0; i < message->enum_type_count(); ++i) {
        CodeGenerator::GenEnumDescriptorGoogProvides(message->enum_type(i), printer);
    }

    // Recursively process nested messages
    for (int i = 0; i < message->nested_type_count(); ++i) {
        CodeGenerator::GenDescriptorGoogProvides(message->nested_type(i), printer);
    }
}

void CodeGenerator::GenEnumDescriptorGoogProvides(const google::protobuf::EnumDescriptor *enum_desc, google::protobuf::io::Printer *printer) {
    printer->Print("goog.provide('$name$');\n",
                   "name", JsFullName(enum_desc->file(), enum_desc->full_name()));
}

void CodeGenerator::GenDescriptor(const google::protobuf::Descriptor *message, google::protobuf::io::Printer *printer) {
    printer->Print("\n"
                   "/**\n"
                   " * Message $name$.\n"
                   " * @constructor\n"
    ...
    printer->Print("$name$ = function() {\n",
                   "name", JsFullName(message->file(), message->full_name()));
    printer->Indent();
    printer->Print("goog.proto2.Message.call(this);\n");
    printer->Outdent();
    printer->Print("};\n"
    ...
    for (int i = 0; i < message->field_count(); ++i) {
        CodeGenerator::GenFieldDescriptor(message->field(i), printer);
    }

    // enums
    for (int i = 0; i < message->enum_type_count(); ++i) {
        CodeGenerator::GenEnumDescriptor(message->enum_type(i), printer);
    }

    // nested messages (recursively process)
    for (int i = 0; i < message->nested_type_count(); ++i) {
        CodeGenerator::GenDescriptor(message->nested_type(i), printer);
        printer->Print("\n\n");
    }
}

void CodeGenerator::GenFieldDescriptor(const google::protobuf::FieldDescriptor *field, google::protobuf::io::Printer *printer) {
    printer->Print("\n");
    std::string type;
    ...
    if (field->type() == google::protobuf::FieldDescriptor::TYPE_BOOL) {
        ...
        type_is_primitive = true;
             ...
             field->type() == google::protobuf::FieldDescriptor::TYPE_SFIXED64) && !as_number)) {
        type = "string";
        type_is_primitive = true;
    }
    else if (field->type() == google::protobuf::FieldDescriptor::TYPE_GROUP || field->type() == google::protobuf::FieldDescriptor::TYPE_MESSAGE) {
        type = JsFullName(field->message_type()->file(), field->message_type()->full_name());
    }
    else if (field->type() == google::protobuf::FieldDescriptor::TYPE_ENUM) {
        type = JsFullName(field->enum_type()->file(), field->enum_type()->full_name());
        type_is_primitive = true;
    }
    else {
        type = "number";
    ...
    if (field->label() == google::protobuf::FieldDescriptor::LABEL_REPEATED) {
        printer->Print("\n"
                       ...
                       "name", field->name());
        printer->Print(" * @return {$opt$$type$} The value.\n"
                       " */\n",
                       "opt", type_is_primitive ? "?" : "",
                       "type", type);
        printer->Print("$prefix$.prototype.get$field$ = function(index) {\n",
                       "prefix", JsFullName(field->containing_type()->file(), field->containing_type()->full_name()),
                       "field", upper_name);
        printer->Indent();
        printer->Print("return /** @type {$opt$$type$} */ ",
                       "opt", type_is_primitive ? "?" : "",
                       "type", type);
        printer->Print("(this.get$$Value($number$, index));\n",
                       "number", number.str());
        printer->Outdent();
        printer->Print("};\n"
                       "\n");
        ...
        printer->Print(" * @return {$opt$$type$} The value.\n"
                       ...
                       "opt", type_is_primitive ? "" : "!",
                       "type", type);
        printer->Print("$prefix$.prototype.get$field$OrDefault = function(index) {\n",
                       "prefix", JsFullName(field->containing_type()->file(), field->containing_type()->full_name()),
                       "field", upper_name);
        printer->Indent();
        printer->Print("return /** @type {$opt$$type$} */ (",
                       "opt", type_is_primitive ? "" : "!",
                       "type", type);
        printer->Print("this.get$$ValueOrDefault($number$, index));\n",
                       "number", number.str());
        printer->Outdent();
        printer->Print("};\n"
                       "\n");
        ...
        printer->Print(" * @return {$opt$$type$} The value.\n"
                       ...
                       "opt", type_is_primitive ? "?" : "",
                       "type", type);
        printer->Print("$prefix$.prototype.get$field$ = function() {\n",
                       "prefix", JsFullName(field->containing_type()->file(), field->containing_type()->full_name()),
                       "field", upper_name);
        printer->Indent();
        printer->Print("return /** @type {$opt$$type$} */ ",
                       "opt", type_is_primitive ? "?" : "",
                       "type", type);
        printer->Print("(this.get$$Value($number$));\n",
                       "number", number.str());
        printer->Outdent();
        printer->Print("};\n"
                       "\n");
        ...
        printer->Print(" * @return {$opt$$type$} The value.\n"
                       ...
                       "opt", type_is_primitive ? "" : "!",
                       "type", type);
        printer->Print("$prefix$.prototype.get$field$OrDefault = function() {\n",
                       "prefix", JsFullName(field->containing_type()->file(), field->containing_type()->full_name()),
                       "field", upper_name);
        printer->Indent();
        printer->Print("return /** @type {$opt$$type$} */ (",
                       "opt", type_is_primitive ? "" : "!",
                       "type", type);
        printer->Print("this.get$$ValueOrDefault($number$));\n",
                       "number", number.str());
        printer->Outdent();
        printer->Print("};\n"
                       "\n");
    ...
    if (field->label() != google::protobuf::FieldDescriptor::LABEL_REPEATED) {
        ...
        printer->Print("$prefix$.prototype.set$field$ = function(value) {\n",
                       ...
                       "field", upper_name);
        printer->Indent();
        printer->Print("this.set$$Value($number$, value);\n",
                       "number", number.str());
        printer->Outdent();
        printer->Print("};\n"
                       "\n");
    ...
    if (field->label() == google::protobuf::FieldDescriptor::LABEL_REPEATED) {
        ...
        printer->Print("$prefix$.prototype.add$field$ = function(value) {\n",
                       ...
                       "field", upper_name);
        printer->Indent();
        printer->Print("this.add$$Value($number$, value);\n",
                       "number", number.str());
        printer->Outdent();
        printer->Print("};\n"
                       "\n");
        ...
        printer->Print("\n"
                       ...
                       "name", field->name());
        printer->Print(" * @return {!Array.<$opt$$type$>} The values in the field.\n"
                       " */\n",
                       "opt", type_is_primitive ? "" : "!",
                       "type", type);
        ...
        printer->Indent();
        printer->Print("return /** @type {!Array.<$opt$$type$>} */ (",
                       "type", type,
                       "opt", type_is_primitive ? "" : "!");
        printer->Print("this.array$$Values($number$));\n",
                       "number", number.str());
        printer->Outdent();
        printer->Print("};\n"
                       "\n");
...
void CodeGenerator::GenEnumDescriptor(const google::protobuf::EnumDescriptor *enum_desc, google::protobuf::io::Printer *printer) {
    printer->Print("\n"
                   "\n"
                   "/**\n"
                   " * Enumeration $name$.\n"
...
void CodeGenerator::GenDescriptorMetadata(const google::protobuf::Descriptor *message, google::protobuf::io::Printer *printer) {
    printer->Print("\n"
                   "\n"
                   " /** @override */\n"
    ...
    printer->Print("$prefix$.descriptor_ = goog.proto2.Message.createDescriptor($prefix$, {\n",
                   "prefix", JsFullName(message->file(), message->full_name()));
    ...
    printer->Print("containingType: $type$,\n",
                   "type", JsFullName(message->containing_type()->file(), message->containing_type()->full_name()));
    }
    ...
    // fields
    for (int i = 0; i < message->field_count(); ++i) {
        CodeGenerator::GenFieldDescriptorMetadata(message->field(i), printer);
    ...
    // nested messages (recursively process)
    for (int i = 0; i < message->nested_type_count(); ++i) {
        CodeGenerator::GenDescriptorMetadata(message->nested_type(i), printer);
    }
}

void CodeGenerator::GenFieldDescriptorMetadata(const google::protobuf::FieldDescriptor *field, google::protobuf::io::Printer *printer) {
    std::ostringstream number;
    number << field->number();
    ...
    if (field->type() == google::protobuf::FieldDescriptor::TYPE_DOUBLE) {
    ...
    else if (field->type() == google::protobuf::FieldDescriptor::TYPE_FIXED64) {
        js_type = "FIXED64";
        if (as_number) {
            default_value << field->default_value_uint64();
        }
    ...
    else if (field->type() == google::protobuf::FieldDescriptor::TYPE_FIXED32) {
        js_type = "FIXED32";
        default_value << field->default_value_uint32();
    }
        ...
        js_type = "ENUM";
        default_value << JsFullName(field->enum_type()->file(), field->enum_type()->full_name() + "." + field->default_value_enum()->name());
    }
    else if (field->type() == google::protobuf::FieldDescriptor::TYPE_SFIXED32) {
        js_type = "SFIXED32";
        default_value << field->default_value_int32();
    }
    else if (field->type() == google::protobuf::FieldDescriptor::TYPE_SFIXED64) {
        js_type = "SFIXED64";
        if (as_number) {
            default_value << field->default_value_int64();
        }
                ...
                || field->type() == google::protobuf::FieldDescriptor::TYPE_SFIXED64) && !as_number))
    {
        ...
        js_object = JsFullName(field->message_type()->file(), field->message_type()->full_name());
    }
    else if (field->type() == google::protobuf::FieldDescriptor::TYPE_ENUM) {
        js_object = JsFullName(field->enum_type()->file(), field->enum_type()->full_name());
    }
    else {
        js_object = "Number";
    }
    ...
    printer->Print("fieldType: goog.proto2.Message.FieldType.$js_type$,\n",
                   "js_type", js_type);
    if (field->has_default_value() || field->type() == google::protobuf::FieldDescriptor::TYPE_ENUM) {
